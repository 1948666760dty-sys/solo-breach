...
display_name: 复杂酒馆
description: 通用、纯文字、长局持续世界互动叙事引擎。用于新开/继续复杂酒馆剧情；v3.5.9 新增真实分支因果与状态域隔离：关键 D2/D3 不能只改 trust/evidence 等数字，必须让至少一部分选择真正改变下一场景、事件顺序、NPC 在场/合作、线索/权限或后续可选行动；专业、调查、资源、身体、关系、亲密等状态域之间也不得无因果串改。继续保留 v3.5.8 真实交互自测、长段默认、v3.5.7 Paragraph Merge Scan、v3.5.6 可见版本确认与 v3.5.5 source-first 开局顺序。
version: 3.5.9
...
# Complex Tavern Engine v3.5.9 — Branch Causality & State-Domain Isolation
...
v3.5.9 是分支因果与状态域隔离修复版本，不改变 v3.5.3 的持久化 schema，也不改变既有故事 Canon、v3.5.8 的真实交互自测/长段默认、v3.5.6 的 Visible Version Confirmation 或 v3.5.5 的 source-first 开局顺序。它针对 100 回合真实玩法审计暴露出的两类隐藏问题做收口：第一，**关键选择必须真实改变至少部分后续路径，而不能只改后台数字后立即回到预设同一事件表**；第二，**专业、调查、资源、身体、关系、亲密等状态域只能在存在明确因果时联动，禁止跨域污染。**

v3.5.8 的交互式自测合同继续保留，并新增关键 D2/D3 的反事实路径检查与状态域归属审计；v3.5.7 的 Paragraph Merge Scan、v3.5.6 的版本确认规则也全部保留。

本次仍沿用 `schema_version: 3.5.3`。新增内容属于导演层、Delta 审计与自测合同，不新增正式故事存档必需字段，因此不需要迁移已有 Canon，也不重写旧 Raw Story Log。

核心目标是：
**让选项真正成为因果输入，而不是装饰；让每个状态变化都能说明“为什么是这个选择导致了这个变化”，从而避免伪分支与跨状态域污染。**